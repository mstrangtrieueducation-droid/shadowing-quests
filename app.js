...
let config, lessons, student, activeLevel = 'all', searchQuery = '', activeStep = 'listen', previousLesson = '', confirmedLessonId = '';
...
function renderCards(){
  const visible=lessons.filter(l=>(activeLevel==='all'||String(l.level)===activeLevel)&&normalize(`${l.id} ${l.title} ${l.topic}`).includes(normalize(searchQuery)));
...
function renderLesson(lesson){
  ...
  document.title=`${lesson.id} · ${lesson.title} | Shadowing`;
  const next=lessons.find(l=>l.level===lesson.level&&l.id>lesson.id);
...
function render(){
  const path=route();
  const lesson=lessons.find(l=>path===`/bai/${l.id}`);
