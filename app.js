...
function renderHeader(){
  header.innerHTML = student ? `<span class="student-chip"><strong>${escape(student.name)}</strong><span>${escape(student.classCode)}</span></span><button class="button button-ghost" id="change-student">Đổi lớp / tên</button>` : '<span class="tag">Mỗi ngày, một bước tiến.</span>';
...
function renderEntry(){
  ...
  main.innerHTML = `<section class="welcome-grid">
    <div class="welcome-copy"><p class="eyebrow">KHÔNG GIAN LUYỆN TẬP · FIGHTER & IELTS</p><h1>Một bài nhỏ.<br>Một bước tiến.</h1><p class="intro">Nghe, luyện và hoàn thành từng quest theo nhịp của bạn. Bắt đầu với ${lessons.length} bài shadowing, mỗi video không quá 6 phút.</p>
    <ol class="steps-list"><li><span class="step-number">01</span><div class="step-copy"><strong>Nghe và hiểu</strong><p>Xem hết video, mở bản lời thoại và hiểu nội dung trước khi luyện.</p></div></li><li><span class="step-number">02</span><div class="step-copy"><strong>Luyện nói toàn bài</strong><p>Nói bám theo giọng gốc. Sửa câu khó, rồi luyện một lượt từ đầu đến cuối.</p></div></li><li><span class="step-number">03</span><div class="step-copy"><strong>Đọc và ghi hình</strong><p>Tắt video gốc. Quay màn hình bản lời thoại kèm micro, đọc toàn bộ phần được giao.</p></div></li><li><span class="step-number">04</span><div class="step-copy"><strong>Nộp đúng bài</strong><p>Mở Form từ bài học. Họ tên, lớp và mã bài sẽ có sẵn; tải lên một video rồi gửi.</p></div></li></ol></div>
    <section class="entry-card" aria-labelledby="entry-title"><p class="card-kicker">SẴN SÀNG BẮT ĐẦU?</p><h2 id="entry-title">Vào lớp của bạn</h2><p>Điền thông tin để mở bài và nộp đúng lớp.</p>${requested?`<p class="inline-notice">Bạn đang mở <strong>${escape(requested.id)}</strong>. Nhập thông tin để tiếp tục đúng bài này.</p>`:''}
      ...
      <p class="entry-note">Chọn lớp giáo viên đã giao cho bạn. Nếu chưa thấy lớp, hãy nhắn giáo viên trước khi làm bài.</p><div class="entry-stats"><span><strong>${lessons.length}</strong> quest</span><span><strong>${new Set(lessons.map(l=>l.level)).size}</strong> level</span><span><strong>1</strong> Form chung</span></div>
    </section></section>`;
...
function renderLibrary(){
  ...
  document.title=`Bài tập ${currentClass().course} · Chuốt`;
  main.innerHTML=`<section class="workspace-head"><div><p class="eyebrow">${escape(student.classCode)} · BÀI TẬP ĐƯỢC GIAO</p><h1>Hôm nay, luyện bài nào?</h1><p class="intro">Mỗi quest là một video đầy đủ. Chọn level giáo viên giao, rồi bắt đầu từ bài ngắn đến bài dài hơn.</p></div><div class="workspace-summary"><strong>${available.length}</strong><span>quest sẵn sàng</span></div></section>
...
function renderCards(){
  ...
  document.querySelector('#result-count').textContent=`${visible.length} bài · Level là mức bài tập, độc lập với lớp Fighter hoặc IELTS.`;
}

function renderLesson(lesson){
  if(previousLesson!==lesson.id){activeStep='listen';previousLesson=lesson.id;}
  document.title=`${lesson.id} · ${lesson.title} | Chuốt`;
  ...
  <aside class="lesson-sidebar"><section class="content-card"><p class="card-kicker">ĐÍCH ĐẾN CỦA BÀI</p><h2>Sau quest này</h2><ul class="instruction-list">${lesson.goals.map(g=>`<li>${escape(g)}</li>`).join('')}</ul></section><section class="submit-card"><span class="tag">1 BÀI · 1 VIDEO</span><h2>Nộp bài của bạn</h2><p><strong>${escape(student.name)}</strong><br>${escape(student.classCode)}<br><span class="lesson-code">${escape(lesson.id)}</span></p><p>Đọc toàn bộ phần lời thoại, quay màn hình kèm micro. Họ tên, lớp và mã bài được điền sẵn trong Form.</p>${externalLink(formUrl(lesson),'Mở Form nộp bài','button button-primary')}<p class="entry-note">Cần đăng nhập Google để tải video. Chỉ hoàn tất sau khi Form báo đã ghi nhận câu trả lời.</p></section>${next?`<a class="text-link next-lesson" href="#/bai/${encodeURIComponent(next.id)}">Bài tiếp theo: ${escape(next.id)} →</a>`:''}</aside></div>`;
...
function renderStage(lesson){
  ...
  if(activeStep==='listen'){
    ...
    target.innerHTML=`<section class="content-card"><p class="card-kicker">PHẦN VIDEO TRÊN TRANG ĐÃ ĐÓNG</p><h2 id="record-title" tabindex="-1">Đọc toàn bài và ghi hình</h2><p>Nếu đã mở video ở tab khác, hãy dừng phát và tắt tiếng ở tab đó trước khi quay. Bản nộp chỉ có giọng đọc của bạn.</p><p class="inline-notice"><strong>Phạm vi đọc đầy đủ:</strong> ${escape(lesson.submissionScope)}</p><p>${escape(lesson.transcriptInstructions)}</p>${externalLink(lesson.transcriptUrl,'Mở bản lời thoại để đọc','button button-secondary')}<ol class="instruction-list"><li>Bật quay màn hình và micro; không cần mở camera.</li><li>Hiển thị rõ bản lời thoại. Đọc liên tục từ đầu đến cuối, gồm lời dẫn, hội thoại, ví dụ và lời kết theo phạm vi của bài.</li><li>Không đọc tên nhân vật, mốc thời gian hoặc đầu/chân trang. Đọc nhầm một từ thì sửa và tiếp tục.</li><li>Xem lại bản quay để chắc có tiếng và đủ lời, rồi nộp một file video qua Form.</li></ol><h3>Trước khi gửi</h3><div class="checklist"><label><input type="checkbox"> Tôi đã đọc đầy đủ phần được giao.</label><label><input type="checkbox"> Màn hình rõ chữ, nghe rõ giọng tôi.</label><label><input type="checkbox"> Video gốc đã tắt trong lúc quay.</label></div>${externalLink(formUrl(lesson),'Mở Form nộp bài','button button-primary')}<p class="entry-note">Mỗi lần nộp một video, tối đa 1 GB. Kiểm tra lại họ tên, lớp và mã bài trong Form trước khi gửi.</p></section>`;
  }
}
function render(){
  renderHeader();
  if(!student){document.title='Vào lớp · Chuốt Fighter & IELTS';renderEntry();return;}
  const path=route();
